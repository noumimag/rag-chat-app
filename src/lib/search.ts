...
import { SearchResult, SearchQuery, Chunk, Document, Vector } from './schema'
...
export class SearchEngine {
    ...
    try {
      ...
      for (const vector of vectors) {
        ...
        if (chunk && document) {
          ...
          if (similarity < 0.1) {
            const keywordScore = this.calculateKeywordRelevance(query.query, chunk.content, document.title)
          ...
          console.log('Query word density boost:', queryWordDensity * 0.1, 'New score:', finalScore)
      ...
      console.log('Results sorted by score, top 3 scores:', results.slice(0, 3).map(r => r.score))

      // Apply MMR (Maximal Marginal Relevance) re-ranking for diversity
      if (results.length > 1) {
        const reRanked = this.mmrReRank(results, query.limit || 10)
        console.log('MMR re-ranking applied, final order:', reRanked.map(r => ({ title: r.document.title, score: r.score })))
        return reRanked
      }
  ...
  private mmrReRank(results: SearchResult[], limit: number, lambda: number = 0.7): SearchResult[] {
    ...
    while (selected.length < limit && remaining.length > 0) {
      ...
      if (bestIndex >= 0) {
        ...
        console.log('MMR: Selected result with score:', selectedResult.score, 'MMR score:', bestScore)
      } else {
        break
      }
    }

    console.log('MMR: Final selection order by score:', selected.map(r => r.score))
    return selected
  }
  ...
  // Search with filters
  async searchWithFilters(query: SearchQuery, filters: {
  ...
  private isAgendaRelatedQuery(query: string): boolean {
    const agendaKeywords = [
      'agenda', 'meeting agenda', 'what was the meeting', 'meeting topics',
      'meeting schedule', 'meeting plan', 'what was discussed', 'meeting outline',
      'when will', 'start date', 'end date', 'term', 'semester', 'schedule',
      'calendar', 'dates', 'beginning', 'first day', 'school year', 'academic year'
    ]
  ...
  private hasAgendaContent(content: string): boolean {
    const agendaIndicators = [
      'agenda:', 'agenda items', 'meeting agenda', 'topics:', 'discussion points',
      'next meeting:', 'meeting adjourned', 'meeting notes', 'standup', 'sprint',
      'term', 'semester', 'start date', 'end date', 'academic year', 'school year',
      'calendar', 'schedule', 'holidays', 'breaks', 'mid-term'
    ]
    const contentLower = content.toLowerCase()
