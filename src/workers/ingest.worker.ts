...
function chunkText(text: string, chunkSize: number = 1200, overlap: number = 200): string[] {
  console.log('Worker: Starting chunking, text length:', text.length, 'chunkSize:', chunkSize, 'overlap:', overlap)
  ...
  try {
    while (startIndex < text.length) {
      console.log('Worker: Processing chunk at startIndex:', startIndex, 'text length:', text.length)
      
      const endIndex = Math.min(startIndex + chunkSize, text.length)
      let chunk = text.substring(startIndex, endIndex)
      
      console.log('Worker: Created chunk, length:', chunk.length, 'startIndex:', startIndex, 'endIndex:', endIndex)
      ...
      if (chunk.trim().length > 150) { // Increased minimum length for better quality
        chunks.push(chunk)
...
async function processDocument(document: Document, chunkSize: number = 1000, overlap: number = 200): Promise<{
  ...
  try {
    ...
    for (let i = 0; i < textChunks.length; i++) {
      ...
      console.log('Worker: Processing chunk', i, 'length:', chunkText.length, 'currentIndex:', currentIndex)
      ...
      if (startIndex >= 0 && endIndex <= document.content.length) {
        ...
        console.warn('Worker: Invalid chunk indices:', { startIndex, endIndex, contentLength: document.content.length, chunkIndex: i })
